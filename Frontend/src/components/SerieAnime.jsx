...
export default class SerieAnime extends React.Component {
  ...
  getList(tipo, clase, start, end){
    ...
    if(this.searchParams.get('s')){
      ...
      if(this.searchParams.get('gen')){
        ...
        if(this.searchParams.get('idioma')){
          ...
          axios.get('http://'+this.ipPc+':5000/getVideoByTypeClassApi/'+this.tipoUrl+'/'+clase+'/'+start+'/'+end)
          .then(res => { 
            this.setState({lista: res.data});
          });
        }
    ...
    axios.get('http://'+this.ipPc+':5000/getGeneroByTypeClassApi/Anime/Serie')
    .then(res => {
      this.setState({listadoGenero: res.data});
    }); 
    axios.get('http://'+this.ipPc+':5000/getTypesByClass/Anime')
    .then(res => {
      this.setState({listadoTipo: res.data});
    });
    axios.get('http://'+this.ipPc+':5000/getLanguagesByTypeClass/Anime/Serie')
    .then(res => {
              ...
              <div key={i} className="portada-box">
                <h2 className="portada-title">
                  <a title={x.Titulo} href={"http://"+this.ipPc+":3000/Video?id="+x.Id+"&cap=1"} rel="bookmark">
                    ...
                    {x.Idioma.map((idi, u) => {
                      return (
                        idi.toLowerCase() == 'latino' ? <img title="Latino" alt="serie anime" style={{"paddingRight":"3px"}} src={require("../images/Latino.png")}/> : 
                        idi.toLowerCase() == 'subtitulado' ? <img title="Subtitulado" alt="serie anime" style={{"paddingRight":"3px"}} src={require("../images/Subtitulado.png")}/> : 
                        <img title={idi} alt="serie anime" style={{"paddingRight":"3px"}} src={require("../images/Desconocido.png")}/>
                      )
                    })}
                    </a>
                </h2>
