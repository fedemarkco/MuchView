...
export default class PeliculaAsiatica extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      lista: [],
  ...
  getList(tipo, clase, start, end){
    ...
    axios.get('https://muchviewapi.herokuapp.com/getGeneroByTypeClassApi/Asiatica/Pelicula')
    .then(res => {
      this.setState({listadoGenero: res.data});
    }); 
    axios.get('https://muchviewapi.herokuapp.com/getLanguagesByTypeClass/Asiatica/Pelicula')
    .then(res => {
      this.setState({listadoIdioma: res.data});
    });
  }
  componentDidMount() {
    this.getList('Pelicula', 'Asiatica', this.pag*18, (this.pag*18)+18);
  }
  createList(title){
    return(
  ...
  render(){
    let peliculaAnime = this.createList('Peliculas Asiática');
    return(
