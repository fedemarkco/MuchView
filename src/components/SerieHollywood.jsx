...
export default class SerieHollywood extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      lista: [],
    ...
    this.params = 'SerieHollywood?pag=';
    this.gen = '';
  ...
  getList(tipo, clase, start, end){
    ...
    axios.get('https://muchviewapi.herokuapp.com/getGeneroByTypeClassApi/Hollywood/Serie')
    .then(res => {
      this.setState({listadoGenero: res.data});
    }); 
    axios.get('https://muchviewapi.herokuapp.com/getLanguagesByTypeClass/Hollywood/Serie')
    .then(res => {
      this.setState({listadoIdioma: res.data});
    });
  }
  componentDidMount() {
    this.getList('Serie', 'Hollywood', this.pag*18, (this.pag*18)+18);
  }
  createList(title){
    return(
              ...
              <div key={i} className="portada-box">
                <h2 className="portada-title">
                  <a title={x.Titulo} href={"http://"+this.ipPc+":3000/Video?id="+x.Id+"&cap=1"} rel="bookmark">
                    ...
                    {x.Idioma.map((idi, u) => {
                      return (
                        idi.toLowerCase() == 'latino' ? <img title="Latino" alt="serie hollywood" style={{"paddingRight":"3px"}} src={require("../images/Latino.png").default}/> : 
                        idi.toLowerCase() == 'subtitulado' ? <img title="Subtitulado" alt="serie hollywood" style={{"paddingRight":"3px"}} src={require("../images/Subtitulado.png").default}/> : 
                        <img title={idi} alt="serie hollywood" style={{"paddingRight":"3px"}} src={require("../images/Desconocido.png").default}/>
                      )
                    })}
                    </a>
                </h2>
  ...
  render(){
    let serieHollywood = this.createList('Series');
    return(
