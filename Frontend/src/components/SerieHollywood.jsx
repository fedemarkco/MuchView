...
export default class SerieHollywood extends React.Component {
  ...
  getList(tipo, clase, start, end){
    ...
    axios.get('http://'+this.ipPc+':5000/getGeneroByTypeClassApi/Hollywood/Serie')
    .then(res => {
      this.setState({listadoGenero: res.data});
    }); 
    axios.get('http://'+this.ipPc+':5000/getLanguagesByTypeClass/Hollywood/Serie')
    .then(res => {
              ...
              <div key={i} className="portada-box">
                <h2 className="portada-title">
                  <a title={x.Titulo} href={"http://"+this.ipPc+":3000/Video?id="+x.Id+"&cap=1"} rel="bookmark">
                    ...
                    {x.Idioma.map((idi, u) => {
                      return (
                        idi.toLowerCase() == 'latino' ? <img title="Latino" alt="serie hollywood" style={{"paddingRight":"3px"}} src={require("../images/Latino.png")}/> : 
                        idi.toLowerCase() == 'subtitulado' ? <img title="Subtitulado" alt="serie hollywood" style={{"paddingRight":"3px"}} src={require("../images/Subtitulado.png")}/> : 
                        <img title={idi} alt="serie hollywood" style={{"paddingRight":"3px"}} src={require("../images/Desconocido.png")}/>
                      )
                    })}
                    </a>
                </h2>
