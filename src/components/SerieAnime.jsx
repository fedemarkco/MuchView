...
export default class SerieAnime extends React.Component {
  constructor(props){
    ...
    this.tipoUrl = 'Serie';
    this.tipo = '';
    if(this.getQueryVariable('tipo')){
      this.tipoUrl =this.getQueryVariable('tipo');
      this.tipo = '&tipo='+this.getQueryVariable('tipo');
    }
    this.idioma = '';
        ...
        <div id="left-content">
          <div className="content-box">
            ...
            <div className="navigation">
              {this.pag == 0 ? '' : <a className="text nav-prev" href={`${process.env.PUBLIC_URL}`+'#/'+this.params+(parseInt(this.pag)-1)+this.search+this.gen+this.tipo+this.idioma}>&laquo; Resultados Anteriores</a>}
              {this.state.lista.length == 18 ? <a className="text nav-prev" href={`${process.env.PUBLIC_URL}`+'#/'+this.params+(parseInt(this.pag)+1)+this.search+this.gen+this.tipo+this.idioma}>Resultados Siguientes &raquo;</a> : ''}
            </div>
          </div>
        </div>
      </div>
