...
export default class ListadoPeliculaHollywood extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      lista: [],
    ...
    if(this.props.search != null){
      ...
      .then(res => { 
        if(res.data.length === 0){
          document.getElementsByClassName('navigation')[0].innerHTML = '<div>No hay resultados para '+this.props.search+'</span></div>';
        }
  ...
  componentDidMount(){
    this.getList('Pelicula', 'Hollywood', 0, 12);
  }
  createList(title){
    return(
              ...
              <div key={i} className="portada-box">
                <h2 className="portada-title">
                  <a title={x.Titulo} href={"http://"+this.ipPc+":3000/Video?id="+x.Id+"&cap=1"} rel="bookmark">
                    ...
                    {x.Idioma.map((idi, u) => {
                      return (
                        idi.toLowerCase() === 'latino' ? <img key={u} title="Latino" alt="pelicula hollywood" style={{"paddingRight":"3px"}} src={require("../images/Latino.png")}/> : 
                        idi.toLowerCase() === 'subtitulado' ? <img key={u} title="Subtitulado" alt="pelicula hollywood" style={{"paddingRight":"3px"}} src={require("../images/Subtitulado.png")}/> : 
                        <img key={u} title={idi} alt="pelicula hollywood" style={{"paddingRight":"3px"}} src={require("../images/Desconocido.png")}/>
                      )
                    })}
                    </a>
                </h2>
