...
export default class SerieDorama extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      lista: [],
  ...
  getList(tipo, clase, start, end){
    ...
    axios.get('https://muchviewapi.herokuapp.com/getGeneroByTypeClassApi/Dorama/Serie')
    .then(res => {
      this.setState({listadoGenero: res.data});
    }); 
    axios.get('https://muchviewapi.herokuapp.com/getLanguagesByTypeClass/Dorama/Serie')
    .then(res => {
      this.setState({listadoIdioma: res.data});
    });
  }
  componentDidMount() {
    this.getList('Serie', 'Dorama', this.pag*18, (this.pag*18)+18);
  }
  createList(title){
    return(
              ...
              <div key={i} className="portada-box">
                <h2 className="portada-title">
                  <a title={x.Titulo} href={"/Video?id="+x.Id+"&cap=1"} rel="bookmark">
                    ...
                    {x.Idioma.map((idi, u) => {
                      return (
                        idi.toLowerCase() == 'latino' ? <img title="Latino" alt="serie dorama" style={{"paddingRight":"3px"}} src={require("../images/Latino.png").default}/> : 
                        idi.toLowerCase() == 'subtitulado' ? <img title="Subtitulado" alt="serie dorama" style={{"paddingRight":"3px"}} src={require("../images/Subtitulado.png").default}/> : 
                        <img title={idi} alt="serie dorama" style={{"paddingRight":"3px"}} src={require("../images/Desconocido.png").default}/>
                      )
                    })}
                    </a>
                </h2>
  ...
  render(){
    let serieDorama = this.createList('Doramas');
    return(
